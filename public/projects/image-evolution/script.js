...
function mutateInt(n, min, max, rate, mutateChance) {	// Returns the value of an integer when mutated based on a set of parameters
	...
	if (mutateChance == 1 || Math.random() < mutateChance) {
...
function randomPoint() {
	return [randInt(imageWidth), randInt(imageHeight)];
}
...
function randomShape() {
	...
	for (var i=0; i<2; i++) {
		newPointX = mutateInt(points[0][0], 0, imageWidth, 0.1, 1);
		newPointY = mutateInt(points[0][1], 0, imageHeight, 0.1, 1);
		points.push([newPointX, newPointY]);
	}
...
class EvolvingImage {
	constructor(shapes=undefined) {
		...
		if (shapes == undefined) {		// Create a new random Evolving Image
			this.shapes = []
			for (var i=0; i<startingShapes; i++) {
	...
	render(context) {	// Renders image on a particular context
		...
		for (var i in this.shapes) {
			context.fillStyle = "rgba(" + this.shapes[i][1][0] + "," + this.shapes[i][1][1] + "," + this.shapes[i][1][2] + ", 0.5)"; 
			context.beginPath();
	...
	getFitness(targetImageData) {	// Returns the fitness when compated to a target image
		this.render(ctxTest);
		var currentImageData = ctxTest.getImageData(0, 0, imageWidth, imageHeight);
	...
	mutate() {
		// Delete shapes
		// If image has atleast 1 shape and 10% chance
		if (this.shapes.length > 0 && Math.random() < removeShapeProbability) {
			var randomIndex = randInt(this.shapes.length-1);
		...
		for (var i in this.shapes) {	// For every shape in the current image class object
			...
			for (var j in this.shapes[i][0]) {		// For each point in this shape
				var newX = mutateInt(this.shapes[i][0][j][0], 0, imageWidth, 0.05, mutateChance);
				var newY = mutateInt(this.shapes[i][0][j][1], 0, imageHeight, 0.05, mutateChance);
				newPoints.push([newX, newY]);
			}

			// For each colour value in this shape
			var newR = mutateInt(this.shapes[i][1][0], 0, 255, 0.05, mutateChance);
			var newG = mutateInt(this.shapes[i][1][1], 0, 255, 0.05, mutateChance);
			var newB = mutateInt(this.shapes[i][1][2], 0, 255, 0.05, mutateChance);
			var newColour = [newR, newG, newB];
			
			this.shapes[i] = [newPoints, newColour];
		//	this.shapes[i][0] = newPoints;
		//	this.shapes[i][1] = newColour;
		}

		// Add new shapes
		// If image can add more shapes and 50% chance
		if (this.shapes.length < curShapes && Math.random() < newShapeProbability) {
			this.shapes.push(randomShape());
		}
...
var maxShapes = 200;
var curShapes = 100;
var startingShapes = 100;

var increaseMaxShapesGens = 1000;

var newShapeProbability = 0.25;
var removeShapeProbability = 0.10;
...
canvasTest.width = imageWidth;
canvasTest.height = imageHeight;
...
// Target Image
var targetImage = new Image(imageWidth, imageHeight);
targetImage.src = "shrekImage.jpg";
...
targetImage.onload = function(){
	...
	ctxTest.drawImage(targetImage, 0, 0, imageWidth, imageHeight);
	/*
	ctxTest.fillStyle = "black";
	ctxTest.fillRect(0, 0, imageWidth, imageHeight);
	ctxTest.fillStyle = "white";
	ctxTest.font = "bold 200px Arial";
	ctxTest.fillText("Send",150,180);
	ctxTest.fillText("Nudes",100,380);
	*/
	targetImageData = ctxTest.getImageData(0, 0, imageWidth, imageHeight);
	console.log(targetImageData);

	document.getElementById("canvas").addEventListener("click", startProgram);

	ctx.font = "bold 50px Arial";
	ctx.textAlign = "center";
	ctx.fillText("Click to start",imageWidth/2,imageHeight/2);
}

function startProgram() {
	document.getElementById("canvas").removeEventListener("click", startProgram);

	// == Genetic algorithm starts here == \\

	// Add starting population to array
	var bestImage = {"fitness":Infinity};
	for (var i=0; i<popSize; i++) {
		var newImage = new EvolvingImage();
		if (newImage.getFitness(targetImageData) < bestImage.fitness) bestImage = newImage;	// Find fitness and compare to bestImage
		population.push(newImage);
	}
	console.log("Starting best fitness is " + bestImage.fitness.toLocaleString());
	bestImage.render(ctx);

	console.log(bestImage);


	window.setInterval(function() {
		if (curShapes<maxShapes && genNum%increaseMaxShapesGens==0) {  // Every 500 generations, add a new circle
			curShapes ++;
		...
		if (newImage.getFitness(targetImageData) < bestImage.fitness) {	// Image has new highest fitness
			bestImage = newImage;
			console.log("Better Fitness in Gen " + genNum + " - " + bestImage.fitness.toLocaleString());
			newImage.render(ctx);
		}
	...
	window.setInterval(function() {
		...
		textString += "<br>Best Fitness: " + bestImage.fitness.toLocaleString();
		textString += "<br>Shapes used: " + bestImage.shapes.length;

		textString += "<br><br>Population Size: " + popSize;
		textString += "<br>Shapes limit: " + curShapes + " / " + maxShapes + " (+1 every " + increaseMaxShapesGens + "gens)";
		
		textString += "<br><br>New Shape Probability: " + (100*newShapeProbability) + "%";
		textString += "<br>Remove Shape Probability: " + (100*removeShapeProbability) + "%";
