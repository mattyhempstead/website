...
class NN {
	constructor(structure, LEARNING_RATE) {
	...
	getOutput(input, finalOutput=false) {	// Returns output of network for a particular input
		...
		let netOutput = new Array(this.structure.length);

		netOutput[0] = [[], input];

		// For each layer (not first because already have input)
		for (let l=1; l<this.structure.length; l++) {
			netOutput[l] = [new Array(this.structure[l]), new Array(this.structure[l])];
			netOutput[l][0].fill(0);
			...
			for (let n=0; n<this.structure[l]; n++) {
				...
				for (let w=0; w<this.structure[l-1]; w++) {
					// Add weighted sum
					netOutput[l][0][n] += netOutput[l-1][1][w] * this.network[l-1][w][n];
				}

				// Add bias to input sum
				netOutput[l][0][n] += this.network[l-1][this.structure[l-1]][n];

				// Apply activation function
				netOutput[l][1][n] = Math.tanh(netOutput[l][0][n]);
			}
		}

		if (finalOutput) {
			return netOutput[this.structure.length-1][1];
		} else {
			return netOutput;
		}
	}
